...
export const jsonld = {
  ...
  product: (p) => ({
    ...
    ...(REVIEWS_VERIFIED && p.rating && p.reviews ? { aggregateRating: { "@type": "AggregateRating", ratingValue: p.rating, reviewCount: p.reviews, bestRating: 5 } } : {}),
...
export function productCard(p) {
  ...
  return `<article class="pcard reveal ${p.stock <= 0 ? "pcard--soldout" : ""}">
    <div class="pcard__media">${art(v, p, { alt: altFor(p, v) })}${badge}</div>
...
export const guaranteeBlock = () => `<div class="guarantee">
  ...
  <p>An unopened jar can be returned any time for a full refund. You arrange and pay the return postage. Honey is food, so once a jar is opened we can't take it back. <a href="/returns/">Read the policy</a></p>
</div>`;
